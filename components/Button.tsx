...
const colorClass = {
  ...
  "disabled": "text-[#a8a8a8] bg-[#f0f0f0] border-[#dcdcdc] cursor-not-allowed dark:text-[#5c5c5c] dark:bg-[#2c2c2c] dark:border-[#3d3d3d]"
}
...
const Button = ({ text, onClick, onHover, variant = 'primary', icon, divClass, title, disabled }: ButtonProps) => {
  ...
  return (
    <div title={title} onClick={onClick} onMouseEnter={onHover} className={`flex items-center justify-center text-[14.5px] py-2.25 px-5.5 rounded-full font-bold border cursor-pointer transition duration-300 ${ disabled ? colorClass.disabled : colorClass[variant] || colorClass.primary} ${divClass}`}>
