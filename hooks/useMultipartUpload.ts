...
async function fetchResume(fileId: string, signal?: AbortSignal): Promise<{
  fileId: string;
  folderId: string;
  uploadId: string;
  token: string;
...
async function resolveSession(file: File, filename: string, folderId: string, contentHash: string = "", signal?: AbortSignal): Promise<SessionResult> {
  const signature = getFileSignature(file, filename);
  const saved = loadPersistedSession(signature);

  if (saved) {
    try {
      const resumed = await fetchResume(saved.fileId, signal);
      if (resumed) {
  ...
  const fresh = await startSession(file, filename, folderId, contentHash, signal);
  savePersistedSession(signature, {
    fileId: fresh.fileId,
    folderId,
...
function attachHash(fileId: string, contentHash: string, signal?: AbortSignal) {
...
export function useMultipartUpload() {
  ...
  const activeControllers = useRef(new Map<File, ActiveUploadEntry>());

  const abortServerSession = (entry: ActiveUploadEntry) => {
    if (!entry.fileId || !entry.uploadId) return;
    fetch("/api/upload/multipart/abort", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      keepalive: true,
      body: JSON.stringify({ fileId: entry.fileId, folderId: entry.folderId, uploadId: entry.uploadId }),
    }).catch(() => { });
  };
  ...
  const cancelPrewarm = useCallback((file: File) => {
    ...
    const active = activeControllers.current.get(file);
    if (active) {
      activeControllers.current.delete(file);
      active.controller.abort();
      abortServerSession(active);
    }
  }, [abortPrewarmedSession]);
  ...
  const upload = useCallback(async (
    ...
    const controllerEntry: ActiveUploadEntry = { controller, fileId: null, folderId: null, uploadId: null };
    activeControllers.current.set(file, controllerEntry);
    ...
    try {
      ...
      const dedupe = await checkDedupe(contentHash, folderId, options.folderName, metadata as DedupeMetadata, signal);
      ...
      const session = canReusePrewarm
        ...
        : await resolveSession(file, filename, folderId, contentHash, signal);

      ({ fileId, uploadId, signature } = session);
      folderId = session.folderId;
      const token = session.token;

      controllerEntry.fileId = fileId;
      controllerEntry.folderId = folderId;
      controllerEntry.uploadId = uploadId;

      await attachHash(fileId, contentHash, signal);
  ...
  const reset = useCallback(() => {
    for (const [file, entry] of activeControllers.current) {
      entry.controller.abort();
      abortServerSession(entry);
      activeControllers.current.delete(file);
    }
